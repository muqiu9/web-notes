...
- 在更新少量节点的时候可以直接向`document.body`节点中添加，大是当要向document中添加大量数据时，如果逐个添加这些新节点，这个过程非常缓慢，因为没添加一个节点都会调用父节点的`appendChild()`方法，为了解决这个问题，可以使用一个文档碎片，把所有的新节点附加其上，然后把文档碎片一次性添加到document中。

## 判断一个节点是否为节点对象

`node.nodeType === 1`则为节点对象
